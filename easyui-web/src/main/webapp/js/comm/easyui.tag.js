 ...
 */
function ajaxPostAdReloadDg2Form(formId, reloadGridId, closeObj,
		reloadGridParams) {
	if ($("#" + formId).form("validate")) {
		showProcess(true, titleinfo, submitTitle);
		$.post($("#" + formId).attr("action"),
				$("#" + formId).serializeArray(), function(data, status) {
					...
					if (!isEmpty(reloadGridParams)) {
						retResultAdReloadByParams4Call(data, reloadGridId,
								closeObj, reloadGridParams);
					} else {
 ...
 *            是否关闭
 */
function ajaxUploadFormCommitAndAutoCallback(formId, reloadGridId, funName,
		closeObj) {
	if ($("#" + formId).form("validate")) {
		showProcess(true, titleinfo, submitTitle);
		$.post($("#" + formId).attr("action"),
				$("#" + formId).serializeArray(), function(data, status) {
					showProcess(false);
					commitUploadFileCallback(data, reloadGridId, funName,
							closeObj);
				});
	}
}
