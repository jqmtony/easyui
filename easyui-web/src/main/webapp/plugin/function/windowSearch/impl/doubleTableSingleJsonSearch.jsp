...
function onDbClickRowDataSelect(){
	...
	<c:choose>
		<c:when test="${requestScope.tableBean.dataType =='1'}">
			 row = $("#"+gridWindowSearch).datagrid("getSelected");	//从表选中行
		</c:when>
		<c:when test="${requestScope.tableBean.dataType =='2'}">
		 	row = $("#"+gridWindowSearch).treegrid("getSelected");	//从表选中行
		</c:when>
		<c:when test="${requestScope.tableBean.dataType =='3'}">
			row = $("#"+gridWindowSearch).tree("getSelected");//从表选中行
		</c:when> 
	</c:choose>

	 //回调函数不为空
	 if(${formListBean.customMethod!=null}){
		 var returnObj;
		 var returnMainObj;
		 ...
		 map.put("mainObj",returnMainObj);
		 map.put("paramsObj",'${formListBean.customMethodParams}');
		 //alert('${formListBean.customMethod}');
		 //alert('${formListBean.customMethodParams}');
...
function onClickMainRowSelect(){
	...
	<c:if test="${!empty tableBeanMain}">	
		...
		for(i=0;i<fieldMain.length;i++){
			params += ((i>0)? "," : "") + fieldChr[i]+":"+eval("row."+fieldMain[i]);		
		}
		<c:choose>
		<c:when test="${requestScope.tableBean.dataType =='1'}">
			reloadDatagridByParams("${requestScope.tableBean.tableName}", params);
			//reloadDatagrid("${requestScope.tableBean.tableName}", url);
		</c:when>
		<c:when test="${requestScope.tableBean.dataType =='2'}">
			//以下方式都要变
			reloadTreegridByParams("${requestScope.tableBean.tableName}", params);
		</c:when>
		<c:when test="${requestScope.tableBean.dataType =='3'}">
			//以后tree也要支持重新加载url
...
<eu:table id="${requestScope.tableBeanMain.tableName}"   idField="${requestScope.tableBeanMain.idField}" 
	url="jsonDo.do?method=toJsonDo&businessName=GETSEARCHPAGE_PAGELISTER&recordFlag=${requestScope.recordFlag }&searchPageId=${requestScope.formListBeanMain.searchPageId }&tableId=${requestScope.tableBeanMain.id}&dataType=${requestScope.tableBeanMain.dataType}&idField=${requestScope.tableBeanMain.idField}&sqlNum=${requestScope.tableBeanMain.sqlNum }&sqlParams=${requestScope.tableBeanMain.sqlParams }"
	 pagination="${requestScope.tableBeanMain.pagination=='1' }" fit="${requestScope.tableBeanMain.fit=='1' }" fitColumns="${requestScope.tableBeanMain.fitColumns=='1' }"	
	 toolbarId="gridToolbarWindowSearch" autoReload="${requestScope.tableBeanMain.isautoReload=='1' }"  stopReload="${requestScope.tableBeanMain.isstopReload=='1' }" singleSelect="true" mergeCellsKeys="${requestScope.tableBeanMain.mergeCellsKeys }"	
	 mergeCellsFields="${requestScope.tableBeanMain.mergeCellsFields }"  onClickRow="onClickMainRowSelect" >
...
<eu:treetable id="${requestScope.tableBeanMain.tableName }"  
 					url="jsonDo.do?method=toJsonDo&businessName=GETSEARCHPAGE_PAGELISTER&recordFlag=${requestScope.recordFlag }&searchPageId=${requestScope.formListBean.searchPageId }&tableId=${requestScope.tableBeanMain.id}&dataType=${requestScope.tableBeanMain.dataType}&idField=${requestScope.tableBeanMain.idField}&sqlNum=${requestScope.tableBeanMain.sqlNum }&sqlParams=${requestScope.tableBeanMain.sqlParams }" 	pagination="${requestScope.tableBeanMain.pagination=='1' }" 
 					 collapsible="${requestScope.tableBeanMain.collapsible=='1' }" fit="${requestScope.tableBeanMain.fit=='1' }" idField="${requestScope.tableBeanMain.idField }" treeField="${requestScope.tableBeanMain.treeField }" singleSelect="true"
		...
		<table class="formTable">
			<jsp:include page="/WEB-INF/plugin/report/commFormTagView.jsp" />	
						<td class="button"  align="center">
							<eu:button id="searchButtonWindowSearch" titleKey="common.button.search" defaultEnterKey="true"
...
<eu:table id="${requestScope.tableBean.tableName}"   idField="${requestScope.tableBean.idField}" 
	url="jsonDo.do?method=toJsonDo&businessName=GETSEARCHPAGE_PAGELISTER&recordFlag=${requestScope.recordFlag }&searchPageId=${requestScope.formListBean.searchPageId }&tableId=${requestScope.tableBean.id}&dataType=${requestScope.tableBean.dataType}&sqlNum=${requestScope.tableBean.sqlNum }&sqlParams=${requestScope.tableBean.sqlParams }" pagination="${requestScope.tableBean.pagination=='1' }"  
		fit="${requestScope.tableBean.fit=='1' }" fitColumns="${requestScope.tableBean.fitColumns=='1' }"	toolbarId="gridToolbarWindowSearch" autoReload="${requestScope.tableBean.isautoReload=='1' }" stopReload="${requestScope.tableBean.isstopReload=='1' }" singleSelect="true"
		mergeCellsKeys="${requestScope.tableBean.mergeCellsKeys }"	mergeCellsFields="${requestScope.tableBean.mergeCellsFields }"  onDbClickRow="onDbClickRowDataSelect" onLoadSuccess="selectRow">
...
<eu:treetable id="${requestScope.tableBean.tableName }"  
 					url="jsonDo.do?method=toJsonDo&businessName=GETSEARCHPAGE_PAGELISTER&recordFlag=${requestScope.recordFlag }&searchPageId=${requestScope.formListBean.searchPageId }&tableId=${requestScope.tableBean.id}&dataType=${requestScope.tableBean.dataType}&idField=${requestScope.tableBean.idField }&sqlNum=${requestScope.tableBean.sqlNum }&sqlParams=${requestScope.tableBean.sqlParams }"  	pagination="${requestScope.tableBean.pagination=='1' }" 
 					 collapsible="${requestScope.tableBean.collapsible=='1' }" fit="${requestScope.tableBean.fit=='1' }" idField="${requestScope.tableBean.idField }" treeField="${requestScope.tableBean.treeField }"  singleSelect="true" 
 					fitColumns="${requestScope.tableBean.fitColumns=='1'}" toolbarId="gridToolbarWindowSearch"   autoReload="${requestScope.tableBean.isautoReload=='1' }"  onDbClickRow="onDbClickRowDataSelect" stopReload="${requestScope.tableBean.isstopReload=='1' }">
	<c:forEach var="bean" 	 items="${requestScope.columnList }" >
	 	<eu:column	 field="${bean.columnName }" 	 title="${bean.columnTitle }"	 width="${bean.width }"  
					 align="${bean.align }" hidden="${bean.ishidden == '1'}"  formatter="${bean.formatter}" />
	</c:forEach> 
