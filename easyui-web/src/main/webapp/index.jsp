...
<div data-options="region:'center'" style="width:90%;height:auto;">
    ...
    <eu:form id="demoFrom" name="demoForm" method="post">
        ...
        <div>
            ...
            <eu:validateBox id="url" name="url" required="true" validType="'url'"/>
        </div>
        <div>
            <label for="email">Email(2数组，应用多个验证):</label>
            <eu:validateBox id="email" name="email" required="true" validType="['email','length[0,5]']"/>
        </div>
        <div>
            <label for="name">Name(3对象，每个key是一个验证名称value是验证的数组参数):</label>
            <eu:validateBox id="name" name="name" required="true" invalidMessage="该工号已存在！" validType="{remote:['${pageContext.request.contextPath}/easyui/user/validateName','name']}"/>
        </div>
    </eu:form>
</div>
</body>
